...
def find_nearest_neighbors(sess_items, sess_time):
    ...
    for itm in usess_items:
        ...
        if len(poss_neigh_idxs) <= K:     # No need to investigate the neighbors.
            itm_knn = poss_neigh_idxs     # K nearest neighbors
        else:
            neigh_indexs = []
            neigh_sims = []
            ...
            for nidx in poss_neigh_idxs:                  # Select each neighbor
                ...
                for nitm in neigh_items:
                    ...
                    item_weight[nitm] =  math.exp(-pos_diff)
                ...
                ''' Factor-2: Sessions closer to current_session are assigned higher weightage'''
                neigh_time = all_train_seq_times[nidx]
                sess_time_diff = abs(sess_time - neigh_time)
                decay = math.exp(-sess_time_diff / (24*60*60))    # Convert to day
                similarity *= decay
            ...
            itm_knn = nidxs[sorted_idcs][:K]         # Sort neigh_indexs based on sorted_idcs.
...
def get_neighbors(name, data, length): 
    ...
    pickle.dump(neigh_data_map, open(dataset + '/' + name + '_neighbors.txt', 'wb'))
    return
