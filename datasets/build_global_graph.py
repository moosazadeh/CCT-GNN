...
max_adj_dist= 2     # Maximum distance of adjacent items. tmall=7, nowplaying=8, diginetica=2
itm_adj_sample = 12
icl = 0.25          # Item Category Lambda. tmall=0.25, nowplaying=0.25, diginetica=0.25
...
def build_graph (name, all_sess_idxs, all_sess_items, neighbors):     
    ''' Choose each session and build initial graph based on its items' relations.
    ...
    for sidx in tqdm(all_sess_idxs):        
        ...
        nidxs = neigh_idxs[0]        # Just take idxs
    ...
    pickle.dump(total_sess_adjs, open(dataset + '/' + name + '_adjs.txt', 'wb'))

    return 
    
print('\ndataset =', dataset, ', itm_adj_sample =', itm_adj_sample, ', max_adj_dist=', max_adj_dist)
print('\n-- Building train sessions global graph --\n')
neighbors = pickle.load(open(dataset + '/train_neighbors.txt', 'rb'))
...
neighbors = pickle.load(open(dataset + '/test_neighbors.txt', 'rb'))
